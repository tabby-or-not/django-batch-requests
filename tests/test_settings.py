...
class TestSettings(TestCase):
    ...
    def test_http_default_content_type_header(self):
        ...
        # Make a batch request querying for that particular header.
        batch_req = self._make_a_batch_request(
            'post', '/echo/?header=%s' % (header), data, headers={}
        )
        batch_resp = json.loads(batch_req.content)[0]
    ...
    def test_duration_header(self):
        ...
        # Make sure we have the header present in enclosing batch response
        # and all individual responses.
        self.assertIn(
            br_settings.DURATION_HEADER_NAME, batch_requests._headers,
            'Enclosing batch request does not contain duration header.'
        )

        self.assertIn(
            br_settings.DURATION_HEADER_NAME, json.loads(batch_requests.content)[0]['headers'],
    ...
    def _make_a_batch_request(self, method, url, body, headers={}):
        ...
        return self.client.post(
            '/api/v1/batch/',
            json.dumps({'batch': [self._batch_request(method, url, body, headers)]}),
    ...
    def _make_multiple_batch_request(self, requests):
        ...
        return self.client.post(
            '/api/v1/batch/',
            json.dumps({'batch': batch_requests}),
            content_type='application/json',
        )
