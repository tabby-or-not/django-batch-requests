...
class TestBadBatchRequest(TestCase):
    ...
    def test_invalid_http_method(self):
        ...
        resp = self.client.post(
            '/api/v1/batch/',
            json.dumps({'batch': [self._batch_request('select', '/views', '', {})]}),
            content_type='application/json'
        )

        self.assertEqual(resp.status_code, 400, 'Method validation is broken!')
        self.assertEqual(
            resp.content.lower(), b'invalid request method.', 'Method validation is broken!'
        )
    ...
    def test_missing_http_method(self):
        ...
        self.assertEqual(
            resp.content.lower(),
            b'request definition should have url, method defined.',
    ...
    def test_invalid_batch_request(self):
        ...
        resp = self.client.post(
            '/api/v1/batch/',
            json.dumps({'batch': {'method': 'get', 'url': '/views/'}}),
            content_type='application/json'
        )
        ...
        self.assertEqual(
            resp.content.lower(),
            b'the body of batch request should always be list!',
    ...
    def test_view_that_raises_exception(self):
        ...
        resp = self.client.post(
            '/api/v1/batch/',
            json.dumps({'batch': [{'method': 'get', 'url': '/exception/'}]}),
            content_type='application/json',
        )
